    ...
    def do_sums1(self, data, sums=None):
        ...
        for i in xrange(nshear):
            if rev[i] != rev[i+1]:
                ...
                g1p = t['mcal_g_1p'].sum(axis=0)
                g1m = t['mcal_g_1m'].sum(axis=0)
                g2p = t['mcal_g_2p'].sum(axis=0)
                g2m = t['mcal_g_2m'].sum(axis=0)

                g1p_psf = t['mcal_g_1p_psf'].sum(axis=0)
                g1m_psf = t['mcal_g_1m_psf'].sum(axis=0)
                g2p_psf = t['mcal_g_2p_psf'].sum(axis=0)
                g2m_psf = t['mcal_g_2m_psf'].sum(axis=0)

                R[0] = (g1p[0]-g1m[0])*factor
                R[1] = (g2p[1]-g2m[1])*factor

                Rpsf[0] = (g1p_psf[0]-g1m_psf[0])*factor
                Rpsf[1] = (g2p_psf[1]-g2m_psf[1])*factor
